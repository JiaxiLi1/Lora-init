...
def parse_args():
    ...
    parser.add_argument("--sparsity", type=float, default=0.05, help="稀疏度（保留列的百分比）")
    parser.add_argument("--gamma", type=float, default=0.5, help="混合参数的初始值")
    parser.add_argument("--sparse_method", type=str, default="svd", choices=["svd", "random"])
    parser.add_argument("--sparse_svd_rank", type=int, default=None)
    parser.add_argument("--cola_silu", type=str_to_bool, default=False)
    parser.add_argument("--cola_init", type=str_to_bool, default=False)
    parser.add_argument("--svd_inverse", type=str_to_bool, default=False)
    parser.add_argument("--lora_alpha", type=float, default=1.0)
    parser.add_argument("--adapter_scope", type=str, default="qv")
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="frequency of exact LORO parameters",
    )
    parser.add_argument(
        "--loro_rank",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="xavier",
        help="xavier | kaiming | orth | randn_x.x | const_x.x",
    )
    parser.add_argument(
        "--loro_scope",
        type=str,
        default=None,
        help="all | qkv | qv | none",
    )
    parser.add_argument(
        "--loro_lr_scaler",
        type=float,
        default=1.0,
        help="scale the learning rate of the low-rank modules",
    )
    parser.add_argument(
        "--loro_alpha",
        type=float,
        default=1.0,
        help="alpha for GeomLowRankAdaptor",
    )
    ...
    if args.output_dir is None:
        ...
        if args.optimizer.lower() == "loro_adamw":
            args.desc = f"loro_adamw_{args.loro_type}"

            assert args.loro_scope is not None
            args.desc += f"_lr_{args.learning_rate}_freq_{args.loro_freq}_r{args.loro_rank}_alpha{args.loro_alpha}"
            args.desc += f"_{args.loro_scope}_{args.loro_init}_rs_{args.loro_lr_scaler}"

        else:
            args.desc = f"{args.optimizer}_r{args.lora_r}"

        args.output_dir = (
            f"./checkpoints/glue_{args.model_name_or_path}/{args.task_name}/{args.desc}"
        )
        print(f"\nOutput_dir = {args.output_dir}\n")

    os.makedirs(args.output_dir, exist_ok=True)
    args.timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    args.log_path = f"{args.output_dir}/log_{args.timestamp}.txt"
...
def main():
    ...
    runname = f"{time.strftime('%m%d_%H%M%S')}_loro_{args.model_name_or_path}_task_{args.task_name}_{args.optimizer}_bs_{args.per_device_train_batch_size}_ep_{args.num_train_epochs}"\
              f"lorarank_{args.lora_rank}_sp_{args.sparsity}_lr_{args.learning_rate}_weight_decay_{args.weight_decay}_alpha_{args.lora_alpha}"
    ...
    # target modules to be finetuned
    ...
    if args.optimizer.lower() == "adamw":
    ...
    elif args.optimizer.lower() == "sparse_adamw":
        ...
        ## TODO: omit all regular parameters
        print("\nOmitting all regular parameters!\n")
        for p in model.parameters():
            if id(p) not in id_galore_params:
        ...
        from loro_torch.lowrank_adpt_module import (
        ...
        param_groups = get_lowrank_adpt_param(model, lr_scaler=args.loro_lr_scaler)
    ...
    print(
        f"\nEpoch 0 before train, eval loss = {eval_loss}, {eval_metric}, time = {time.time() - eval_start_time}s\n"
    )

    best_eval_metric = eval_metric.copy()

    wandb_dict_tmp = {
        "eval_loss": eval_loss,
        "eval_dataset_len": len(eval_dataloader.dataset),
        "eval_samples": samples_seen,
        "eval_time": time.time() - eval_start_time,
    }
    wandb_dict_tmp.update({f"eval_{k}": v for k, v in eval_metric.items()})
    wandb_dict_tmp.update({f"best_eval_{k}": v for k, v in best_eval_metric.items()})
    wandb.log(wandb_dict_tmp, step=0)
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(active_dataloader):
            ...
            if (
                ...
                ## NOTE: adapt for geomlrk
                if args.optimizer.lower() == "loro_adamw":
                    ## use Riemannian update every loro_freq steps
                    use_exact_loro = (completed_steps + 1) % args.loro_freq == 0
                    optimizer.step(use_exact_loro=use_exact_loro)
                else:
                    use_exact_loro = False
                    optimizer.step()
                ...
                verbose_str = f"Epoch {epoch} | Step {completed_steps} | LR {lr_tmp:.6f} | Loss {loss.item():.4f}"
                if args.optimizer.lower() == "loro_adamw":
                ...
                df_train_stats = pd.concat(
                    [df_train_stats, df_train_tmp], ignore_index=True
                )
                df_train_stats.to_csv(
                    os.path.join(args.output_dir, "train_stats.csv"), index=False
                )
                ...
                wandb.log(wandb_dict_tmp, step=completed_steps)

            if isinstance(checkpointing_steps, int) and args.save_ckpt:
        ...
        eval_metric = metric.compute()
        print(
            f"\nEpoch {epoch}, eval loss = {eval_loss}, {eval_metric}, time = {time.time() - eval_start_time}s\n"
        )

        del metric

        # update eval stats
        df_eval_tmp = eval_metric.copy()
        df_eval_tmp["eval_loss"] = eval_loss.item()
        df_eval_tmp["epoch"] = epoch
        df_eval_tmp["step"] = completed_steps
        df_eval_tmp = pd.DataFrame({k: [v] for k, v in df_eval_tmp.items()})
        df_eval_stats = pd.concat([df_eval_stats, df_eval_tmp], ignore_index=True)
        df_eval_stats.to_csv(
            os.path.join(args.output_dir, "eval_stats.csv"), index=False
        )

        print(eval_metric)
        print(best_eval_metric)
        best_eval_metric = {
            k: max(v, best_eval_metric[k]) for k, v in eval_metric.items()
        }
        wandb_dict_tmp = {
            "eval_loss": eval_loss,
            "eval_dataset_len": len(eval_dataloader.dataset),
        ...
        wandb_dict_tmp.update({f"eval_{k}": v for k, v in eval_metric.items()})
        wandb_dict_tmp.update(
            {f"best_eval_{k}": v for k, v in best_eval_metric.items()}
        )
        wandb.log(wandb_dict_tmp, step=completed_steps)
        ...
        if args.push_to_hub and epoch < args.num_train_epochs - 1 and args.save_ckpt:
            accelerator.wait_for_everyone()
        ...
        if args.checkpointing_steps == "epoch" and args.save_ckpt:
    ...
    if args.output_dir is not None and args.save_ckpt:
        accelerator.wait_for_everyone()
        unwrapped_model = accelerator.unwrap_model(model)
        unwrapped_model.save_pretrained(
            args.output_dir,
